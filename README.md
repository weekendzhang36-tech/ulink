...
该命令只创建缺失的职业规划、实习实践、金融底色、文化交流分类，`¥5 / 半年` 友邻成长计划，基础服务入口和少量新闻/活动式示例内容；已存在的数据不会重复创建，也不会覆盖后台已有修改。它不会在部署或启动时自动运行，`NODE_ENV=production` 时默认拒绝执行；如果生产环境确实要使用这批首发数据，需要先确认内容后设置 `MVP_SEED_ALLOW_PRODUCTION=true`。
...
首次提交资料成功后，小程序会进入“资料已提交”页，展示当前学生姓名、学校、班级和“待认证”状态，并提供“接收认证结果通知”和“进入首页”两个动作。后续从“我的”或内容门禁进入时，继续使用“认证状态”页查看状态、修改资料或重新提交。

“我的”页会展示友邻成长计划会员状态、有效期和当前成长计划权益明细。权益文案从 PayloadCMS 成长计划配置读取，不能在小程序端硬编码成与后台不一致的固定文案。
