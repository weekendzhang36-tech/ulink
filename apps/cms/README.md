...
- `NODE_ENV=production` 时，本地 mock 登录、手机号、短信、支付下单和模拟支付回调会被后台拒绝，不能作为生产绕过方式。
- 订单、会员、支付事件、学生认证、审核日志、内容预约和导入数据必须以后端持久化状态为准。
- 指导员认证列表只能返回当前手机号负责班级内的学生，并支持按状态和负责班级过滤；学生摘要返回可读校园名称和提交时间，避免小程序展示内部关系 ID。
- 学生编辑资料时，只有新注册或手机号变更才需要新的手机号验证 token；未变更已验证手机号时，不重复要求授权。
- 成长计划会员下单和继续支付必须要求学生认证已通过；有效会员不能重复创建新的成长计划订单；订单查询、订单列表和取消待支付订单仍只校验当前学生自己的订单归属。
- 成长计划接口返回小程序读模型，只包含标题、说明、价格、有效期和权益等展示/支付字段，不直接透出 Payload 系统字段。
- 内容接口返回小程序读模型，内容类型同时保留机器字段和学生可读中文文案，避免页面展示 `event`、`opportunity` 等后台配置值。
- 内容详情可允许待认证学生浏览公开信息，但报名、预约和会员内容行动必须要求学生认证已通过；未通过时不创建预约记录。
- 服务入口支持配置外部链接、第三方小程序或咨询承接说明，小程序按后台配置执行，不把第三方能力伪装成自研功能。
