...
- 模块内容和服务入口请求会带当前登录态；未登录或未完成资料时进入统一登录/资料引导，不展示成长服务列表。
- 指导员认证管理页支持按状态和班级筛选学生，列表展示学校、学院、专业、班级、提交时间和“待认证 / 已认证 / 需确认”状态。
- 会员订单页支持取消待支付订单，取消结果以后端订单状态为准；已支付、已关闭和已取消订单只展示只读状态。
- “我的预约”支持读取当前学生自己的预约记录，并可取消预约；取消以后以后端状态和名额释放结果为准。
- “我的”页会员卡会读取当前成长计划配置展示权益明细；权益文案来自后台，不在小程序端写死。
...
- 学生端运行态文案不展示“后台”等内部实现词；页面文案用“平台记录、服务开放情况、已开放”等学生可理解表达。
- 资料页会保存本地草稿并在提交成功后清理；草稿不保存验证码、手机号验证 token 或已验证状态。
- 首次提交资料成功后会进入“资料已提交”页，展示姓名、学校、班级和待认证状态，并提供“接收认证结果通知”和“进入首页”两个动作。
- 编辑资料页会把已保存手机号视为已验证；未改手机号时可直接提交，切换或更换手机号后必须重新微信授权或短信验证。
